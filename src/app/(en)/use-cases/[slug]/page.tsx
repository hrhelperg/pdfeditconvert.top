...
import { getUseCaseContent } from "@/content/registry";
import { UseCasePage } from "@/components/sections/UseCasePage";

const LOCALE = "en" as const;
const SECTION = "use-cases" as const;
...
export default async function Page({
  ...
  const entry = sectionEntry(LOCALE, SECTION, slug);
  if (!entry) notFound();
  const content = getUseCaseContent(LOCALE, entry.englishSlug);
  if (!content) notFound();
  return <UseCasePage content={content} locale={LOCALE} />;
}
